...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="workspace">
      <header className="mobile-header"><div className="mobile-account"><Brand /><button onClick={async () => { if (remoteMode) await api.logout(); setAuthenticated(false); }}><LogOut size={17} />Sair</button></div><details><summary><Laptop size={22} /><span><strong>{selected.name}</strong><small className={selected.online ? "state-positive" : "state-negative"}>{selected.online ? "Online" : "Offline"}</small></span><ChevronDown size={19} /></summary><div>{devices.map((device) => <button key={device.id} onClick={() => setSelectedId(device.id)}>{device.name}<small className={device.online ? "state-positive" : "state-negative"}>{device.online ? "Online" : "Offline"}</small></button>)}</div></details></header>
      <header className="workspace-header"><div><Monitor size={29} /><span><h1>{selected.name}</h1><small className={selected.online ? "state-positive" : "state-negative"}>{selected.online ? "Online" : "Offline"}</small></span></div><button onClick={() => void load()}><RefreshCw size={17} />{lastSeen(selected.last_seen_at)}</button></header>
      <div className={`workspace-body ${view === "communication" ? "communication-workspace" : ""}`}>
        {view === "now" && <Now device={selected} onBonus={() => setModal("bonus")} onPause={() => void command(selected.monitoring_paused ? "resume_monitoring" : "pause_monitoring", (d) => ({ ...d, monitoring_paused: !d.monitoring_paused }), selected.monitoring_paused ? "Contagem retomada." : "Contagem pausada.")} onBlock={() => void command(selected.manual_block ? "clear_manual_block" : "block_now", (d) => ({ ...d, manual_block: !d.manual_block }), selected.manual_block ? "Computador desbloqueado." : "Bloqueio solicitado.")} />}
...
function Now({ device, onBonus, onPause, onBlock }: { device: Device; onBonus: () => void; onPause: () => void; onBlock: () => void }) {
  const remaining = device.remaining_seconds; const next = device.routines.find((r) => r.enabled);
  const [now, setNow] = useState(() => new Date());
  useEffect(() => { const timer = window.setInterval(() => setNow(new Date()), 30000); return () => window.clearInterval(timer); }, []);
  const minutes = now.getHours() * 60 + now.getMinutes(); const daySegments = [-1, 0, 1].map((offset) => ({ offset, when: ["ontem", "hoje", "amanhã"][offset], segments: routineSegmentsForDay(device.routines, (now.getDay() + offset + 7) % 7) }));
  return <section className="rhythm-board"><h2 className="section-name">Agora</h2><div className="time-hero"><strong>{fmt(remaining)}</strong><span>{device.manual_block ? "acesso bloqueado" : device.monitoring_paused ? "contagem pausada" : "restantes hoje"}</span></div><div className="day-track"><div className="day-window"><div className="day-strip" style={{ transform: `translateX(${(-(minutes / 1440) * 100 - 50) / 3}%)` }}>{daySegments.map(({ offset, when, segments }) => <div className={offset === 1 ? "day-cell" : "day-cell adjacent"} key={offset}><div className="track-labels"><span>00:00</span><span>06:00</span><b>12:00</b><span>18:00</span><span /></div>{segments.length > 0 && <div aria-label={`Rotinas de bloqueio de ${when}`} className="routine-bands">{segments.map((segment) => { const start = clock(segment.start); const end = segment.end === 86400 ? "24:00" : clock(segment.end); const description = `${segment.name}: bloqueio das ${start} às ${end}`; return <span aria-label={`${description} de ${when}`} key={segment.key} role="img" style={{ left: `${(segment.start / 86400) * 100}%`, width: `${((segment.end - segment.start) / 86400) * 100}%` }} title={description} />; })}</div>}<div className="ticks" /></div>)}</div></div><i aria-hidden="true" /></div>{daySegments.some(({ segments }) => segments.length > 0) && <div className="track-legend"><span aria-hidden="true" /><LockKeyhole size={13} />Rotinas de bloqueio</div>}<dl className="time-facts"><div><dt>Usado</dt><dd>{fmt(device.used_seconds)}</dd></div><div><dt>Limite</dt><dd>{fmt(device.today_quota_seconds + device.bonus_seconds)}</dd></div></dl><div className="quick-actions"><button className="primary" onClick={onBonus}><Plus size={20} />Mais tempo</button><button onClick={onPause}>{device.monitoring_paused ? <Play size={20} /> : <Pause size={20} />}{device.monitoring_paused ? "Retomar" : "Pausar"}</button><button onClick={onBlock}>{device.manual_block ? <UnlockKeyhole size={20} /> : <LockKeyhole size={20} />}{device.manual_block ? "Desbloquear" : "Bloquear"}</button></div><div className="detail-grid"><DetailSection title="Resumo de hoje" Icon={CalendarDays}><Row Icon={Clock3} label="Próxima rotina" value={next ? `${next.name} · ${clock(next.start_second)}` : "Nenhuma ativa"} /><Row Icon={RefreshCw} label="Última sincronização" value={lastSeen(device.last_seen_at)} /></DetailSection><DetailSection title="Estado do computador" Icon={Monitor}><Row Icon={ShieldCheck} label="Agente" value={device.online ? "Conectado" : "Desconectado"} /><Row Icon={UserRoundCheck} label="Sessão gráfica" value={device.graphical_session_active ? "Ativa" : "Sem sessão"} /><Row Icon={MonitorCheck} label="Monitoramento" value={device.monitoring_paused ? "Pausado" : "Ativo"} /><Row Icon={Clock3} label="Contagem de tempo" value={device.counting ? "Em andamento" : "Parada"} /></DetailSection></div></section>;
}
...
function statusTone(value: string) {
  if (["Conectado", "Ativa", "Ativo", "Em andamento", "Configurada"].includes(value)) return "state-positive";
  if (["Pausado", "Aguardando"].includes(value)) return "state-warning";
  if (["Desconectado", "Offline", "Sem sessão", "Inativo", "Parada", "Não configurada", "Bloqueado"].includes(value)) return "state-negative";
  return "";
}
...
function Limits({ device, onSave }: { device: Device; onSave: (weekly: number[]) => Promise<void> }) {
  ...
  return <section className="editor-page"><header><div><h2>Limites</h2><p>Defina o tempo disponível em cada dia da semana.</p></div></header><div className="day-tabs">{dayNames.map((name, index) => <button className={day === index ? "active" : ""} key={name} onClick={() => setDay(index)}><span>{name}</span><strong>{fmt(draft[index])}</strong></button>)}</div><section className="limit-editor"><div className="limit-value"><span>{dayNames[day]}</span><strong>{fmt(value)}</strong><small>em intervalos de 15 minutos</small></div><input aria-label={`Limite de ${dayNames[day]}`} min="0" max="86400" step="900" type="range" value={value} onChange={(event) => setDraft((all) => all.map((item, index) => index === day ? Number(event.target.value) : item))} /><div className="step-actions"><button onClick={() => setDraft((all) => all.map((item, index) => index === day ? 0 : item))}>Bloquear o dia</button><button onClick={() => setDraft((all) => all.map((item, index) => index === day ? 86400 : item))}>Liberar o dia</button></div><button className="primary-button" disabled={busy} onClick={async () => { setBusy(true); try { await onSave(draft); } finally { setBusy(false); } }}><Save size={18} />{busy ? "Salvando…" : "Salvar limites"}</button></section></section>;
}
function Routines({ device, onNew, onToggle, onDelete }: { device: Device; onNew: () => void; onToggle: (routine: Routine) => Promise<void>; onDelete: (routine: Routine) => Promise<void> }) {
  return <section className="editor-page"><header><div><h2>Rotinas</h2><p>Crie ritmos automáticos para estudo, descanso e lazer.</p></div><button className="primary-button" onClick={onNew}><Plus size={18} />Nova rotina</button></header><div className="routine-list">{device.routines.length ? device.routines.map((routine) => <article key={routine.id}><span className="routine-time">{clock(routine.start_second)}<i />{clock(routine.end_second)}</span><div><h3>{routine.name}</h3><p>{routine.days.map((selected, index) => selected ? dayNames[index] : "").filter(Boolean).join(" · ")}</p></div><button aria-pressed={routine.enabled} className={`switch ${routine.enabled ? "active" : ""}`} onClick={() => void onToggle(routine)}><i /></button><button aria-label={`Excluir ${routine.name}`} className="delete" onClick={() => void onDelete(routine)}><Trash2 size={18} /></button></article>) : <div className="empty"><CalendarDays size={28} /><h3>Nenhuma rotina criada</h3><p>Comece organizando um período recorrente.</p></div>}</div></section>;
}
...
function DeviceModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (name: string) => Promise<void> }) { const [name, setName] = useState(""); const [busy, setBusy] = useState(false); return <Modal title="Novo computador" description="Dê um nome simples antes de parear o agente." onClose={onClose}><form className="modal-form" onSubmit={async (event) => { event.preventDefault(); setBusy(true); try { await onSubmit(name.trim()); } finally { setBusy(false); } }}><label>Nome<input autoFocus placeholder="Ex.: Notebook da Ana" value={name} onChange={(event) => setName(event.target.value)} /></label><div className="modal-actions"><button type="button" onClick={onClose}>Cancelar</button><button className="primary" disabled={!name.trim() || busy}><Plus size={18} />Adicionar</button></div></form></Modal>; }
function RoutineModal({ routines, onClose, onSubmit }: { routines: Routine[]; onClose: () => void; onSubmit: (routine: Omit<Routine, "id">) => Promise<void> }) { const [name, setName] = useState(""); const [start, setStart] = useState("18:30"); const [end, setEnd] = useState("20:00"); const [days, setDays] = useState([1,2,3,4,5]); const [busy, setBusy] = useState(false); const [serverError, setServerError] = useState(""); const draft = { name: name.trim(), start_second: seconds(start), end_second: seconds(end), days: dayNames.map((_, index) => days.includes(index)) as Routine["days"], enabled: true }; const conflict = conflictingRoutine(draft, routines); const error = conflict ? `Este intervalo já está ocupado pela rotina “${conflict.name}”.` : serverError; return <Modal title="Nova rotina" description="Escolha um nome, horários e dias." onClose={onClose}><form className="modal-form" onSubmit={async (event) => { event.preventDefault(); if (conflict) return; setBusy(true); setServerError(""); try { await onSubmit(draft); } catch (submitError) { setServerError(submitError instanceof Error ? submitError.message : "Não foi possível criar a rotina."); } finally { setBusy(false); } }}><label>Nome<input autoFocus placeholder="Ex.: Tempo de estudo" value={name} onChange={(event) => setName(event.target.value)} /></label><div className="time-pair"><TimePicker label="Começa" value={start} onChange={(value) => { setStart(value); setServerError(""); }} /><TimePicker label="Termina" value={end} onChange={(value) => { setEnd(value); setServerError(""); }} /></div><fieldset className="weekday-picker"><legend>Dias</legend>{dayNames.map((day, index) => { const active = days.includes(index); return <button aria-pressed={active} className={active ? "active" : ""} key={day} onClick={() => { setDays((all) => all.includes(index) ? all.filter((item) => item !== index) : [...all, index]); setServerError(""); }} type="button">{active && <Check size={14} />}{day}</button>; })}</fieldset>{error && <div className="routine-conflict-alert" role="alert"><AlertTriangle aria-hidden="true" size={20} /><div><strong>{conflict ? "Horário indisponível" : "Não foi possível criar a rotina"}</strong><span>{error}</span></div></div>}<div className="modal-actions"><button type="button" onClick={onClose}>Cancelar</button><button className="primary" disabled={!name.trim() || !days.length || Boolean(conflict) || busy}><CalendarDays size={18} />Criar rotina</button></div></form></Modal>; }
