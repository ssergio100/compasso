...
export function CommunicationPage({ deviceId, deviceName }: { deviceId: string; deviceName: string }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; window.clearInterval(interval); polling.current = false; };
  }, [deviceId]);
  ...
  const visible = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      if (!term) return true;
      const searchable = [routeText(event), operationName(event.operation), event.summary, ...Object.values(event.details)].join(" ").toLocaleLowerCase("pt-BR");
  ...
  return <section className="communication-page">
    ...
    <div className="log-table" role="table" aria-label={`Comunicação de ${deviceName}`}>
      <div className="log-table-header" role="row"><span>Hora local</span><span>Origem</span><span>Operação</span><span>Resultado</span><span>Duração</span><span>Resumo</span><span /></div>
      {visible.map((event) => <div className={`log-entry ${event.result}`} key={event.id}>
        <button aria-expanded={expanded === event.id} className="log-row" onClick={() => setExpanded((current) => current === event.id ? null : event.id)} role="row">
          ...
          <span>{routeText(event)}</span>
          <strong>{operationName(event.operation)}</strong>
          <b className={`log-result ${event.result}`}>{resultText(event)}</b>
          <span>{event.duration_ms ? `${event.duration_ms.toLocaleString("pt-BR")} ms` : "—"}</span>
          <span>{event.summary}</span>
          {expanded === event.id ? <ChevronDown aria-hidden="true" size={18} /> : <ChevronRight aria-hidden="true" size={18} />}
        </button>
        {expanded === event.id && <div className="log-details"><dl>{Object.entries(event.details).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value || "—"}</dd></div>)}</dl><p>Campos sensíveis foram omitidos.</p></div>}
      </div>)}
