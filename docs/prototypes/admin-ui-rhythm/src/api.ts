import type { Device, DeviceDetailResponse, DeviceResponse, Routine, Session } from "./types";

const runtimeBase = window.COMPASSO_CONFIG?.apiBaseUrl?.trim() ?? "";
const envBase = (import.meta.env.VITE_COMPASSO_API_BASE_URL ?? "").trim();
export const remoteMode = import.meta.env.VITE_COMPASSO_REMOTE === "true" || Boolean(runtimeBase || envBase);

class API {
  private csrf = "";
  private base = import.meta.env.DEV ? "" : runtimeBase || envBase;
  ...
  async devices() {
    ...
    return Promise.all(list.devices.map(async (summary): Promise<Device> => {
      const detail = await this.request<DeviceDetailResponse>(`/api/v1/admin/devices/${summary.id}`);
      return {
        id: summary.id, name: summary.name, online: detail.status.online,
        ...
        last_seen_at: summary.last_seen_at, weekly_quota_seconds: detail.policy.weekly_quota_seconds,
  ...
  bonus(id: string, minutes: number) { return this.request(`/api/v1/admin/devices/${id}/bonus`, { method: "POST", body: JSON.stringify({ minutes }) }, true); }
